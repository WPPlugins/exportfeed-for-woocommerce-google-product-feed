...
function GCPF_check_version()
{
    ...
    if (!isset($plugin_info->response[GCPF_PLUGIN_BASENAME]))
        return ' | You are up to date';

    $CPF_WP_version = $plugin_info->response[GCPF_PLUGIN_BASENAME]->new_version; //wordpress repository version
    //version_compare:
    //returns -1 if the first version is lower than the second,
    //0 if they are equal,
    //1 if the second is lower.
    $doUpdate = version_compare($CPF_WP_version, GCPF_FEED_PLUGIN_VERSION);
...
function GCPF_print_info()
{
    ...
    echo
        ...
        . '<br>Version: ' . GCPF_FEED_PLUGIN_VERSION . GCPF_check_version() . '<br>
	 	</div>
	 	<div class="gts-link">
...
function GCPF_render_navigation()
{
    $active_tab = isset($_GET['tab']) ? $_GET['tab'] : 'createfeed';
    $tutorials_url = site_url() . '/wp-admin/admin.php?page=gcpf-feed-tutorials-page';
    $url = site_url() . '/wp-admin/admin.php?page=gcpf-feed-manage-page';
    ?>
    <div class="nav-wrapper">
        <nav class="nav-tab-wrapper">
            <a href="<?php echo site_url(); ?>/wp-admin/admin.php?page=gcpf-feed-admin&tab=createfeed"
               class="nav-tab <?php echo $active_tab == 'createfeed' ? 'nav-tab-active ' : ''; ?>"><?php _e('Create Feed', 'gcpf-feed-strings'); ?></a>
            <a href="<?php echo $url; ?>&tab=managefeed"
               class="nav-tab <?php echo $active_tab == 'managefeed' ? 'nav-tab-active ' : ''; ?>"><?php _e('Manage Feed', 'gcpf-feed-strings'); ?></a>
            <a href="http://www.exportfeed.com/contact/"
               target="_blank"
               class="nav-tab <?php echo $active_tab == 'contactus' ? 'nav-tab-active ' : ''; ?>"><?php _e('Contact Us', 'gcpf-feed-strings'); ?></a>
            <a href="<?php echo $tutorials_url; ?>&tab=tutorials"
               class="nav-tab <?php echo $active_tab == 'tutorials' ? 'nav-tab-active ' : ''; ?>"><?php _e('Tutorials', 'gcpf-feed-strings'); ?></a>

            <a href="http://www.exportfeed.com/woocommerce-product-feed/" target="_blank"
               class="nav-tab"><?php _e('Go Pro', 'gcpf-feed-strings'); ?></a>
