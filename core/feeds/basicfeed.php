...
class GCPF_PBasicFeed
{
    ...
    public function addAttributeDefault($attributeName, $value, $defaultClass = 'GCPF_PAttributeDefault')
    {
        ...
        //$thisDefault = new GCPF_PAttributeDefault();
    ...
    {
        $className = 'GCPF_PFeedRule' . ucwords(strtolower($ruleClass));
    ...
    protected function continueFeed($category, $file_name, $file_path, $remote_category, $miinto_country_code = '')
    {
        ...
        if ($this->has_footer && $done) {
            ...
            GCPF_PFeedActivityLog::updateFeedList($category, $remote_category, $file_name, $file_path, $this->providerName, $this->productCount, $miinto_country_code);
        }
    ...
    protected function continueFeedCustom($category, $file_name, $file_path, $remote_category)
    {
        ...
        if ($this->has_footer && $done) {
            ...
            fwrite($this->fileHandle, $this->getFeedFooter($file_name, $file_path));
            $this->productCount = $this->updateObject->productCount;
            GCPF_PFeedActivityLog::updateFeedList($category, $remote_category, $file_name, $file_path, $this->providerName, $this->productCount);
        }
        fclose($this->fileHandle);
    }
    ...
    function getFeedData($category, $remote_category, $file_name, $saved_feed = null, $miinto_country_code = null)
    {
        ...
        $file_url = GCPF_PFeedFolder::uploadFolder() . $this->providerName . '/' . $file_name . '.' . $this->fileformat;
        $file_path = GCPF_PFeedFolder::uploadURL() . $this->providerName . '/' . $file_name . '.' . $this->fileformat;
        ...
        if (($gfcore->cmsName == 'WordPress') && (strpos($file_path, 'https://') !== false) && (strpos(content_url(), 'https') === false))
            $file_path = str_replace('https://', 'http://', $file_path);
        $this->file_path = $file_path;

        //Shipping and Taxation systems
        $this->shipping = new GCPF_PShippingData($this);
        $this->taxData = new GCPF_PTaxationData($this);

        $this->logActivity('Initializing categories...');

        //Figure out what categories the user wants to export
        $this->categories = new GCPF_PProductCategories($category);
        //Get the ProductList ready
        if ($this->productList == null)
            $this->productList = new GCPF_PProductList();
        ...
        foreach ($this->relatedData as $relatedData)
            if (!isset($this->productList->relatedData[$relatedData[1]]))
                $this->productList->relatedData[$relatedData[1]] = new GCPF_PProductSupplementalData($relatedData[0]);
    ...
    function getCustomFeedData($file_name, $saved_feed = null, $miinto_country_code = null)
    {
        ...
        //Figure out what categories the user wants to export
        //$this->categories = new GCPF_PProductCategories($cats);
        ...
        if ($this->auto_update_feedlist)
            GCPF_PFeedActivityLog::updateCustomFeedList($cpf_custom_local_category, $cpf_custom_remote_category, $file_name, $file_path, $this->providerName, $this->productCount, $miinto_country_code);
    ...
    function initializeOverrides($saved_feed)
    {
        ...
        $this->feedOverrides = new GCPF_PFeedOverride($this->providerName, $this, $saved_feed);
    ...
    function loadAttributeUserMap()
    {
        ...
        global $gfcore;
        $map_string = $gfcore->settingGet('cpf_attribute_user_map_' . $this->providerName);
        if ($map_string == '[]') { //if map_string is not object //temp fix for backwards compatibility... true fix below
            $gfcore->settingSet('cpf_attribute_user_map_' . $this->providerName, '');
            $map_string = '';
        }
    ...
    function __construct($saved_feed = null)
    {
        ...
        $this->feed_category = new GCPF_md5y();
        $this->weight_unit = $gfcore->weight_unit;
        $this->dimension_unit = $gfcore->dimension_unit;
        $this->currency = $gfcore->currency;
...
//********************************************************************
// GCPF_PXMLFeed has functions an XML Feed would need
//********************************************************************

class GCPF_PXMLFeed extends GCPF_PBasicFeed
{
...
//********************************************************************
// GCPF_PXML2Feed has functions an XML Feed would need
// The product level element contains the id (ex: <auction id=“1294”>)
//********************************************************************

class GCPF_PXML2Feed extends GCPF_PBasicFeed
{
...
//********************************************************************
// GCPF_PCSVFeed has functions a CSV Feed would need
//********************************************************************

class GCPF_PCSVFeed extends GCPF_PBasicFeed
{
...
class GCPF_PTSVFeed extends GCPF_PBasicFeed
{
...
//********************************************************************
// GCPF_PCSVFeedEx has functions a CSV Feed would need
// but phasing out deprecated functions
//********************************************************************

class GCPF_PCSVFeedEx extends GCPF_PBasicFeed
{
...
class GCPF_PTSVFeedEx extends GCPF_PBasicFeed
{
...
class GCPF_PAggregateFeed extends GCPF_PBasicFeed
{
    ...
    function initializeAggregateFeed($id, $file_name)
    {
        ...
        $this->filename = GCPF_PFeedFolder::uploadFolder() . $this->providerName . '/' . $file_name . '.' . $this->fileformat;
        $this->file_url = GCPF_PFeedFolder::uploadURL() . $this->providerName . '/' . $file_name . '.' . $this->fileformat;
        $this->productCount = 0;
        $this->file_name_short = $file_name;

        if (!isset($this->feeds) || count($this->feeds) == 0) {
            global $gfcore;
            $data = $gfcore->settingGet('cpf_aggrfeedlist_' . $id);
            $data = explode(',', $data);
            $this->feeds = array();
