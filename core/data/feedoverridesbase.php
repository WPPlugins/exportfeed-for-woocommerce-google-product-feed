...
class GCPF_PBaseFeedOverride
{

    protected $advancedCommands = array();
    protected $advancedCommandsCustom = array();
    ...
    {
        global $gfcore;
        $loadDropDownMappings = 'loadDropDownMappings' . $gfcore->callSuffix;
    ...
    private function loadDropDownMappingsJ($providerName)
    {
        ...
        $db->setQuery('
			...
			WHERE name LIKE ' . $db->quote($providerName . '_cp_%')
        );
    ...
    private function loadDropDownMappingsJH($providerName)
    {

        $this->loadDropDownMappingsJ($providerName);
    ...
    {
        global $gfcore;
        $loadAdvancedCommands = 'loadAdvancedCommands' . $gfcore->callSuffix;
    ...
    {
        global $gfcore;
        $loadAdvancedCommands = 'loadCustomAdvancedCommands' . $gfcore->callSuffix;
        $this->$loadAdvancedCommands($providerName);
    }
    ...
    private function loadAdvancedCommandsJ($providerName)
    {
        ...
        $db->setQuery('
			...
			WHERE name = ' . $db->quote($providerName . '-google-feed-settings')
        );
        $data = $db->loadResult();
        if (strlen($data) > 0)
    ...
    private function loadAdvancedCommandsJH($providerName)
    {

        $this->loadAdvancedCommandsJ($providerName);
    ...
    private function loadAdvancedCommandsJS($providerName)
    {
        ...
        $db->setQuery("
			SELECT value FROM #__cartproductfeed_options
			WHERE (name = '" . $providerName . "-google-feed-settings') AND (shop_id = $shopID)");
        $data = $db->loadResult();
        if (strlen($data) > 0)
            $this->advancedCommands = explode("\n", $data);
    }
    ...
    {
        //Advanced options
        $this->advancedCommands = explode("\n", get_option($providerName . '-google-feed-settings'));
    }

    private function loadCustomAdvancedCommandsW($providerName)
    {
        $this->advancedCommandsCustom = get_option($providerName . '-google-feed-settings-custom');
    }
