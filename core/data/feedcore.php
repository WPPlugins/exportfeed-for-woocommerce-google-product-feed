...
class GCPF_PFeedCore
{
    ...
    function settingDeleteJS($settingName)
    {
        ...
        $db->setQuery('
			DELETE
			FROM #__cartproductfeed_options
			WHERE name = ' . $db->quote($settingName) . ' AND (shop_id = ' . $gfcore->shopID . ')');
        $result = $db->loadResult();
        return $result;
    }
    ...
    function settingGetJS($settingName)
    {
        ...
        if ($gfcore->shopID == -1)
            ...
            $shopCondition = ' AND (shop_id = ' . $gfcore->shopID . ')';
        $db = JFactory::getDBO();
        $db->setQuery('
			SELECT value
			FROM #__cartproductfeed_options
    ...
    {
        if (!isset($gfcore->dispatcher))
            $gfcore->dispatcher = JEventDispatcher::getInstance();
        $results = $gfcore->dispatcher->trigger($eventname, array($param1, $param2, $param3));
    }
