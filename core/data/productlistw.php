...
class GCPF_PAProductW extends GCPF_PAProduct
{

}

class GCPF_PMasterAttributeList
{
...
class GCPF_PProductList
{
    ...
    public function loadProducts($parent)
    {
        ...
        if (!isset($this->relatedData['brand']) && $parent->allowRelatedData)
            $this->relatedData['brand'] = new GCPF_PProductSupplementalData('product_brand');
        if (!isset($this->relatedData['tag']) && $parent->allowRelatedData)
            $this->relatedData['tag'] = new GCPF_PProductSupplementalData('product_tag');
    ...
    {
        $this->masterAttributeList = new GCPF_PMasterAttributeList();
        global $wpdb;
    ...
    public function getProductListDefault($parent, $remote_category)
    {
        ...
        foreach ($this->products as $index => $prod) {
            ...
            //Prepare the item
            $item = new GCPF_PAProductW(); //extends GCPF_PAProduct (basicfeed.php)
            ...
            if ($parent->allow_attributes)
                ...
                $item->attribute_details = array();
            $item->taxonomies = new GCPF_PTaxonomyList();
            foreach ($item->attribute_details as $this_attribute) {
                $this_attribute = explode('=', $this_attribute);
                if (count($this_attribute) > 1) {
    ...
    public function getProductListCustom($parent, $remote_category)
    {
        ...
        global $wpdb,$woocommerce,$gfcore;
        $wc_version = explode(",",$woocommerce->version);
        $parent->logActivity('Retrieving product list from database');
        if ($this->products == null)
    ...
    function hideStockValid($item)
    {
        global $gfcore;
        //Hide out of stock
        //if ( ($gfcore->manage_stock) )
        if (($gfcore->hide_outofstock) && ($item->attributes['stock_status'] == 0))
    ...
    { //if variable product
        ...
        foreach ($resultlist as $index => $item) {
            ...
            $this->hideStockValid($item);
            //Hide out of stock
            // if (($gfcore->manage_stock) && ($gfcore->hide_outofstock) && ($item->attributes['stock_status'] == 0))
            // 	$item->attributes['valid'] = false;
