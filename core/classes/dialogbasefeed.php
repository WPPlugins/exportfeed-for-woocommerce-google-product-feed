...
class GCPF_PBaseFeedDialog
{
    ...
    {
        $found_options = new GCPF_FoundOptions($this->service_name, $thisAttribute);
        $output = '
    ...
    function attributeMappings()
    {
        ...
        global $gfcore;
        $FoundAttributes = new GCPF_FoundAttribute();
        ...
        */
        ...
        $output .= '
                    <div class="feeds_by_category" id="cpf_feeds_by_category"><div class="cpf_feed_by_category_left">
                        <p>Use the drop downs below to re-map ' . $gfcore->cmsPluginName . ' attributes to ' . $this->service_name . '\'s required attributes.<br>
...
$output .='</div>';
    ...
    function loadFeedConfig(){
        global $wpdb;
        $this->feed_config = get_option($this->service_name. '-google-feed-settings-custom');
    }
    ...
    public function initializeProvider()
    {
        ...
        $providerName = 'GCPF_P' . $this->service_name . 'Feed';
    ...
    {
         global $gfcore;
        $this->advancedSettings = $gfcore->settingGet($this->service_name.'-google-feed-settings');
         if ($source_feed == null) {
        ...
        $attrVal = array();
        $this->folders = new GCPF_PFeedFolder();
        $this->product_categories = new GCPF_PProductCategories(); //used?
