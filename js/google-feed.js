...
function parseUploadFeedResultStatus(data, id) {
    data.action = gcpf.action;
    data.security = gcpf.gcpf_nonce;
...
function doUpdateSetting(source, settingName) {
    ...
    jQuery.ajax({
        type: "post",
        url: ajaxurl,
        data: "action="+gcpf.action+"&security="+gcpf.gcpf_nonce+"&feedpath="+cmdUpdateSetting+"&setting=" + settingName + unique_setting + "&shop_id=" + shopID + "&value=" + jQuery("#" + source).val(),
        success: function (res) {
